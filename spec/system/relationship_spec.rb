...
RSpec.describe "Relationships", type: :system do
  ...
  describe "フォロー中(following一覧)ページ" do
    ...
    context "ページレイアウト" do
      ...
      it "ユーザー情報が表示されていること" do
        ...
        expect(page).to have_content "本#{user.books.count}件"
  ...
  describe "フィード" do
    ...
    it "フィードに自分の投稿が含まれていること" do
      expect(user.feed).to include book
    end

    it "フィードにフォロー中ユーザーの投稿が含まれていること" do
      expect(user.feed).to include book2
    end

    it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
      expect(user.feed).not_to include book3
    end
  end
end
