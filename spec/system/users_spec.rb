...
RSpec.describe "Users", type: :system do
  ...
  describe "ユーザー登録ページ" do
    ...
    context "ユーザー登録処理" do
      it "有効なユーザーでユーザー登録を行うとユーザー登録成功のフラッシュが表示されること" do
        ...
        click_button "登録する"
        expect(page).to have_content "BOOK TIMEへようこそ！"
      end
  ...
  describe "プロフィール編集ページ" do
    ...
    it "有効なプロフィール更新を行うと、更新成功のフラッシュが表示されること" do
      ...
      expect(user.reload.sex).to eq "編集：女性"
    end
  ...
  describe "プロフィールページ" do
    context "ページレイアウト" do
      ...
      it "本の件数が表示されていることを確認" do
        expect(page).to have_content "本 (#{user.books.count})"
      end

      it "本の情報が表示されていることを確認" do
    ...
    context "通知生成" do
      ...
      context "自分以外のユーザーの本の感想に対して" do
        ...
        it "お気に入り登録によって通知が作成されること" do
          ...
          expect(page).to have_css 'li.no_notification'
          expect(page).to have_content "#{user.name}さんにお気に入り登録されました。"
          expect(page).to have_content other_book.name
          expect(page).to have_content other_book.thoughts
          expect(page).to have_content other_book.created_at.strftime("%Y/%m/%d(%a) %H:%M")
        end
        ...
        it "コメントによって通知が作成されること" do
          ...
          visit notifications_path
          expect(page).to have_css 'li.no_notification'
          expect(page).to have_content "#{user.name}さんがコメントしました。"
      ...
      context "自分の本の感想に対して" do
        ...
        it "お気に入り登録によって通知が作成されないこと" do
          ...
          expect(page).not_to have_content book.thoughts
          expect(page).not_to have_content book.created_at
        end
        ...
        it "コメントによって通知が作成されないこと" do
          ...
          expect(page).not_to have_content other_book.thoughts
          expect(page).not_to have_content other_book.created_at
        end
      end
    end
  end
